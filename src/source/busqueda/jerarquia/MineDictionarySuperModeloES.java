...
/**
 * Versión con diccionario, supermodelo y estrategias Express y Save
 * @author vanesa.graino
 *
 */
public class MineDictionarySuperModeloES extends
      MineDictionarySuperModeloExpress {

   protected static final String TIEMPOS_ESTRATEGIA = "estrategia";
   private static final Logger LOGGER = Logger.getLogger(MineDictionarySuperModeloES.class.getName());
   ...
   public MineDictionarySuperModeloES(String executionId,
   ...
   /*
    * (non-Javadoc)
    * @see source.busqueda.jerarquia.MineDictionarySuperModelo#calcularSoporteTam3(java.util.List)
    */
   @Override
   protected void calcularSoporteTam3(IColeccion coleccion){
      ...
      for(ISecuencia secuencia : coleccion){
         ...
         for(Evento evento : secuencia){
            ...
            if(antes && ventanaActual.isEmpty()){ eventosSinAnotaciones[tamActual-1]++; }
         }
         sid++;
         finSecuencia();
      }
   ...
   protected void calcularSoporteGeneral(IColeccion coleccion, int tamActual){
      ...
      for(ISecuencia secuencia : coleccion){
         ...
         for(Evento evento : secuencia){
            ...
            //Collections.sort(patternIDs); //esto ya no es necesario por la estrategia SAVE
            supermodelo.actualizaVentana(sid,evento);
   ...
   /**
    * Para faciliar MineDictionarySuperMarcarArbol
    */
   ...
   protected void setAnotacionesEvento(List<Patron> encontrados, List<Patron> ventanaActual,
      ...
      //MineAhorro.setAnotacionesEvento(encontrados, ventanaActual, evento, tam,
      //      raizArbol, anotacionesEvitadas, tiemposEstrategia);
      setAnotacionesEvento(encontrados, ventanaActual, evento, tam,
            raizArbol, anotacionesEvitadas, registroT);
   }
         ...
         for(i=0; i<nSize; i++){
            ...
            for(j=i+1;j<nSize;j++){
               ...
               if(nodoAux == null){
                  ...
                  modArray = nodoAux.getModelo().getTipos(); //para no volver a intanciar el modelo
               }
