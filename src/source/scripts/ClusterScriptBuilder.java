 ...
 *
 */
public class ClusterScriptBuilder extends ScriptBuilder {

    private static final Logger LOGGER = Logger.getLogger(ClusterScriptBuilder.class.getName());

    int indice = 1;

    public ClusterScriptBuilder(String pathCodigo){
        super(pathCodigo);
    }
    ...
    private class SingleClusterScriptBuilder extends AbstractClusterScriptBuilder{
        ...
        public SingleClusterScriptBuilder(ConfScriptBuilder c, ConfigSintetica bd,
            ...
            this.ppn = 8;//Para asegurarnos de que os traballos entran en //ExecutionParameters.CLUSTER_PPN;
            this.numNodes = 1;//ExecutionParameters.CLUSTER_NODES;
            //this.mem = "15gb";
        ...
        protected void escribirContenido(BufferedWriter fScript) throws IOException {
            fScript.write(commandoEjecucionSintetica(c, bd, alg, windowSize, null, null));
        }
           ...
           for(int numBD=0; numBD<c.bases.size()-1; numBD++){
              ...
              for(int windowSize : bd.windows){
                  SingleClusterScriptBuilder single = new SingleClusterScriptBuilder(c,
                          bd, alg, windowSize);
                  single.pathGeneracionScripts= c.scriptPath + "/cluster/" + bd.nombre;
    ...
    public static void main(final String[] args) {
       ...
       new ClusterScriptBuilder("").escribirScripts(c);
    }
