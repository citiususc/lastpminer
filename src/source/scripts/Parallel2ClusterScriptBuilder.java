 ...
 *
 */
public class Parallel2ClusterScriptBuilder extends ScriptBuilder {

    private static final Logger LOGGER = Logger.getLogger(Parallel2ClusterScriptBuilder.class.getName());

    int indice = 1;

    public Parallel2ClusterScriptBuilder(String pathCodigo){
        super(pathCodigo);
    }
    ...
    private class SingleClusterScriptBuilder extends AbstractClusterScriptBuilder{
        ...
        public SingleClusterScriptBuilder(ConfScriptBuilder c, ConfigSintetica bd,
                String alg, int windowSize, int iterations){
            this.c = c;
            this.bd = bd;
            this.alg = alg;
            ...
            this.ppn = 64;//iterations; //ExecutionParameters.CLUSTER_PPN;
            this.walltime = "12:00:00";
        ...
        protected void escribirContenido(BufferedWriter fScript) throws IOException {
           for(int i=0;i<iterations;i++){
            fScript.write(commandoEjecucionSintetica(c, bd, alg, windowSize, i, null) + " &");
            fScript.newLine();
           }
           fScript.write("wait");
           fScript.newLine();
           ...
           for(int numBD=0; numBD<c.bases.size()-1; numBD++){
              ...
              for(int windowSize : bd.windows){
                  SingleClusterScriptBuilder single = new SingleClusterScriptBuilder(c,
                          bd, alg, windowSize, c.iterations);
                  single.pathGeneracionScripts= c.scriptPath + "/parallel2_cluster/" + bd.nombre;
    ...
    public static void main(final String[] args) {
        ...
        c.writeSingleReport = false;
        c.writeTxtReport = true;
        c.writeReport = false;
        c.appendResults = false;
       new Parallel2ClusterScriptBuilder("").escribirScripts(c);
    }
