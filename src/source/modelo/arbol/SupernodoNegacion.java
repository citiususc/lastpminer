 ...
 * un prefijo para los modelos negativos {@code IAsociacionConNegacion.PREF_NEG}.
 * @author vanesa.graino
 *
 */
public class SupernodoNegacion extends Supernodo {

   public SupernodoNegacion() {
   }

   public SupernodoNegacion(Nodo padre) {
      super(padre);
   }
   ...
   public Nodo obtenerNodoEnArbol(List<String> tiposPositivos, List<String> tiposNegativos){
      ...
      for(String tipoNeg : tiposNegativos){
         String tipo = IAsociacionConNegacion.PREF_NEG + tipoNeg;
         if(actual==null){ return null; }
         resultado = actual.getHijo(tipo);
         if(resultado==null){ return null; }
         actual = resultado.getHijos();
   ...
   protected Nodo removeNodo(String tipo, boolean esPositivo){
      Nodo nodo = nodos.remove(esPositivo? tipo : IAsociacionConNegacion.PREF_NEG + tipo);
      lista.remove(nodo);
      return nodo;
   }

   public Nodo getHijo(String tipo, boolean esPositivo){
      return nodos.get(esPositivo? tipo : IAsociacionConNegacion.PREF_NEG + tipo);
   }


   public void addNodo(Nodo nodo, String tipo, boolean esPositivo){
      nodos.put(esPositivo? tipo : IAsociacionConNegacion.PREF_NEG + tipo,nodo);
      lista.add(nodo);
   }
